...
export default class Lesson extends React.Component {
  ...
  getUnlockedLessons(subLevelId, lessonId) {
    ...
    if (this.state.user) {
      LessonStore.getUserLessons(this.state.user._id, subLevelId, (err, response) => {
        if (err) {
          return
        }
        ...
        if(!this.state.isTeacher){
          filteredLesson = response.unlockedLessons.filter((unlockedLesson) => {
  ...
  getLessonInfo(lessonId) {
    ...
    LessonStore.getOne(lessonId, (err, response) => {
      ...
      PhrasesActions.getPhraseTranslations(lesson.phraseId, { phrases: lesson.phraseId },
  ...
  unlockPractices(enabledPractices, lessonId, paginationData) {
    PracticeStore.getUserPractices(this.state.user._id, lessonId, (error, practiceResponse) => {
      let practices = [];
      if (practiceResponse) {
        ...
        practices = enabledPractices.map((practice) => {
          ...
          practiceResponse.unlockedPractices.filter((unlockedPractice, index) => {
  ...
  practiceDetails(index) {
    ...
    // se obtiene la traducción de la phrase
    PhrasesActions.getPhraseTranslations(practice.phraseId, { phrases: practice.phraseId },
      (err, phraseTranslations) => {
        if (phraseTranslations[0]) {
          this.setState({
  ...
  unlockSections(enabledSections, practiceId, paginationData) {
    SectionStore.getUserSections(this.state.user._id, practiceId, (error, sectionResponse) => {
      let sections = [];
      ...
      if (sectionResponse) {
        ...
        sections = enabledSections.map((section) => {
          ...
          sectionResponse.unlockedSections.filter((unlockedSection, index) => {
  ...
  commentsFinished() {
    this.setState({ commentsFinished: true }, (() => {
  ...
  exercisesFinished() {
    this.setState({ exercisesFinished: true }, (() => {
      this.checkIsAllFinished();
    }).bind(this));
  }
  ...
  checkIsAllFinished() {
    ...
    } else if ((this.state.currentSection.name == Constants.SECTION.GRAMMAR ||
      this.state.currentSection.name == Constants.SECTION.PRONUNCIATION ||
      this.state.currentSection.name == Constants.SECTION.CONVERSATION) && this.state.exercisesFinished) {
      this.unlockNextSection()
    }
  }
  ...
  unlockNextSection() {
    ...
    let currentSectionIndex = this.state.allSections.indexOf(this.state.currentSection);
    console.log('currentSectionIndex', currentSectionIndex);
    console.log('this.state.allSections.length - 1', this.state.allSections.length - 1);
    if (currentSectionIndex == this.state.allSections.length - 1) {
      this.unlockNextPractice();
    } else {
      SectionStore.unlockNextSection(this.state.currentPractice._id, this.state.currentSection._id,
        (err, response) => {
  ...
  unlockNextPractice() {
    ...
    // se busca el indice de la sección que se terminó
    let currentPracticeIndex = this.state.allPractices.indexOf(this.state.currentPractice);
    // se compara si la sección es la ultima es por que se debe desbloquear la siguiente practica
    console.log('currentPracticeIndex', currentPracticeIndex)
    console.log('this.state.allPractices.length - 1', this.state.allPractices.length - 1)
    if (currentPracticeIndex == this.state.allPractices.length - 1) {
      this.unlockNextLesson();
    } else {
      // se envia la practica y la seccion a ser desbloqueada
      console.log("this.state.currentPractice._id", this.state.currentPractice._id)
      console.log("this.props.params.id", this.props.params.id)
      PracticeStore.unlockNextPractice(this.state.currentPractice._id, this.props.params.id,
        (err, response) => {
          if (err) console.log(err)
          this.setState(update(this.state, {
            allPractices: {
            ...
            description: {
              $set: `has completado exitosamente la práctica ${this.state.allPractices[currentPracticeIndex].name} continua con la practica ${this.state.allPractices[currentPracticeIndex + 1].name}`
            },
            translation: {
              $set: "Felicitaciones!"
            }
  ...
  unlockNextLesson() {
    ...
    let lastLessonId = this.state.allLessons[this.state.allLessons.length - 1]._id;
    ...
    if (currentLessonId == lastLessonId) {
      ...
      // se envia la practica y la seccion a ser desbloqueada
      LessonStore.unlockNextLesson(this.props.params.subLevelId, currentLessonId,
        (err, response) => {
          if (err) console.log(err)
  ...
  unlockNextSubLevel() {
    ...
    let lastSubLevelId = this.state.allSubLevels[this.state.allSubLevels.length - 1]._id;
    ...
    if (lastSubLevelId == currentSubLevelId) {
      ...
      // se envia la practica y la seccion a ser desbloqueada
      SubLevelStore.unlockNextSubLevel(this.props.params.levelId, currentSubLevelId,
        (err, response) => {
          if (err) console.log(err)
          // se redirecciona al usuario a la pagina principal
  ...
  unlockNextLevel() {
    ...
    // se envia la practica y la seccion a ser desbloqueada
    LevelStore.unlockNextLevel(this.props.params.levelId,
      (err, response) => {
        if (err) console.log(err)
        // se redirecciona al usuario a la pagina principal
        this.context.router.push('/user-area/practice-web/');
  ...
  createInteractions() {
    ...
    for (let interaction of this.state.allInteractions) {
      if (interaction.interactionType == 1 || interaction.interactionType == 2) {
    ...
    if (commentInteraction.length >= 2) {
      ...
      for (var index = 0; index < commentInteraction.length;) {
        ...
        if (commentInteraction[index + 1] != null) {
          newInteractions = this.sequence1(JSON.parse(JSON.stringify(commentInteraction[index])), JSON.parse(JSON.stringify(commentInteraction[index + 1])), newInteractions)
        } else {
          break
        }
        index += 2;
  ...
  newObjConst(interaction, author, type) {
    ...
    if (interaction.castell)
      this.castell = interaction.castell
      ...
      <div style={{ background: "#F6F7F7" }}>
                  ...
                  <div className="col-xs-12 front">
                    ...
                    <div className="row">
                      <div className="col-xs-12 col-md-4">
                        <div className="row">
                          <div className="col-xs-12 front-flex mousePoint" onClick={this.showPractices.bind(this)}>
                            <div className="practices">{this.state.pageTexts[2]}</div>
                        ...
                        if (this.state.showPractices) {
                          return (
                            <div className="col-xs-12 col-md">
                              <div className="row">
                                {this.state.allPractices.map((practice, index) => {
                                  return <PracticeItem
                                    pageTexts = {this.state.pageTexts}
                                    ...
                                    description={practice.description}
                                    onPracticeClicked={this.practiceDetails.bind(this, index)}
                                    practiceSelected={this.practiceSelected.bind(this)}
                                    itemSelected={this.state.practiceSelected}
        ...
        {(() => {
          if (this.state.currentSection) {
            if (this.state.currentSection.name == Constants.SECTION.VOCABULARY) {
              console.log('this.state.allComments VOCABULARY', this.state.allComments);
              return (
                <VocabularySectionItem allComments={this.state.allComments}
              ...
              || this.state.currentSection.name == Constants.SECTION.PRONUNCIATION) {
              return (
                <GrammarSectionItem allInteractions={this.state.allInteractions}
                  toCongratulate={this.toCongratulate.bind(this)}
                  showFailMessage={this.toError.bind(this)}
                  exercisesFinished={this.exercisesFinished.bind(this)}
                  pageTexts={this.state.pageTexts}
            ...
            } else if (this.state.currentSection.name == Constants.SECTION.CONVERSATION) {
              ...
              return (
                ...
                <ConversationSectionItem allInteractions={newInteractions}
                  showFailMessage={this.toError.bind(this)}
                  toCongratulate={this.toCongratulate.bind(this)}
                  exercisesFinished={this.exercisesFinished.bind(this)}
                />
...

        {
          this.state.currentSection &&
            <div className="col-xs-12 action-container" style={{paddingRight: '3rem'}}>
              <button className="solution-button mousePoint back-button" onClick={this.unlockNextSection.bind(this)}>Pasar al siguiente ejercicio</button>
            </div>
        }
        ...
        <div className={"to-congratulate " + this.state.showError}>
          <div className="col-xs-12 congrat-image">
            <img className="to-congratulate-img" src={"/images/" + this.state.currentError + ".png"} />
          </div>
        </div>
      </div>
    )
