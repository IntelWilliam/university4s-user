...
class TranslateContainer extends React.Component {
  ...
  loadTranslations() {
    ...
    console.log('this.state.exercise.phraseId', this.state.exercise.phraseId);
      ...
      (err, phraseTranslations) => {
        ...
        if (phraseTranslations.length > 0) {
          let translations = phraseTranslations.map((translations) => {
            return removeDiacritics(translations.phrases.phrase).replace(
              /([^ña-zA-z]+)/gi,
              ''
            );
          });
  ...
  checkIfFinished(phrase) {
    ...
    let phraseWithoutAccents = removeDiacritics(phrase).replace(
      /([^ña-zA-Z]+)/gi,
      ''
    );
        ...
        <div className='col-xs-12'>
          <div className='order-word-container'>
                  ...
                  if (this.state.castell) {
                    return (
                      <span>
                        ...
                        <span className='right-translation-phrase blue-text'>
                          ({sentenceCase(this.state.castell)})
                        </span>
                      </span>
                    );
                  }
            ...
            <div className='words-to-translate'>
              <TranslateInput
                ref='input'
                isOk={this.state.isPhraseOk}
                checkIfFinished={this.checkIfFinished.bind(this)}
              />
            </div>
            <div className='row'>
              <div className='col-xs-12'>
