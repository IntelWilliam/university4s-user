    ...
    loadExercise(interactionId) {
        ...
        ExercisesStore.getAll(params, (error, exercise) => {
            if(error) {}
                ...
                this.props.exerciseDescription.call(null,exerciseData, this.state.exerciseNumber, this.props.number)
            }
        })
    }
            ...
            <div className="col-xs-12">
                {(() => {
                    if(this.state.currentExercise) {
                        if(this.state.currentExercise.exerciseType == Constants.EXERCISE.WORD_IMAGE) {
                            return (
                                <Container exercise={this.state.currentExercise}
                                           showFailMessage={this.props.showFailMessage}
                                           loadNext={this.loadNext.bind(this)} />
                            )
                        } else if(this.state.currentExercise.exerciseType == Constants.EXERCISE.ORDER_WORDS) {
                            return (
                                <OrderWordContainer exercise={this.state.currentExercise}
                                                    showFailMessage={this.props.showFailMessage}
                                                    loadNext={this.loadNext.bind(this)} />
                            )
                        } else if(this.state.currentExercise.exerciseType == Constants.EXERCISE.CORRECT_TRANSLATION) {
                            return (
                                <RightTranslationContainer exercise={this.state.currentExercise}
                                                           showFailMessage={this.props.showFailMessage}
                                                           loadNext={this.loadNext.bind(this)}/>
                            )
                        } else if(this.state.currentExercise.exerciseType == Constants.EXERCISE.MISSING_WORD) {
                            console.log('this.state.currentExercise', this.state.currentExercise);
                            return (
                                <ListenCompleteContainer exercise={this.state.currentExercise}
                                                         showFailMessage={this.props.showFailMessage}
                                                         loadNext={this.loadNext.bind(this)}/>
                            )
                        } else if(this.state.currentExercise.exerciseType == Constants.EXERCISE.ERASE_WORD) {
                            return (
                                <EraseWordContainer exercise={this.state.currentExercise}
                                                    showFailMessage={this.props.showFailMessage}
                                                    loadNext={this.loadNext.bind(this)}/>
                            )
                        } else if(this.state.currentExercise.exerciseType == Constants.EXERCISE.TRANSLATE_WORD) {
                            return (
                                <TranslateContainer exercise={this.state.currentExercise}
                                                    showFailMessage={this.props.showFailMessage}
                                                    loadNext={this.loadNext.bind(this)}/>
                            )
                        } else if(this.state.currentExercise.exerciseType == Constants.EXERCISE.LISTEN_WRITE) {
                            return (
                                <ListenWriteContainer exercise={this.state.currentExercise}
                                                      showFailMessage={this.props.showFailMessage}
                                                      loadNext={this.loadNext.bind(this)}/>
                            )
                        } else if(this.state.currentExercise.exerciseType == Constants.EXERCISE.CORRECT_ANSWER) {
                            return (
                                <CorrectAnswerContainer exercise={this.state.currentExercise}
                                                        showFailMessage={this.props.showFailMessage}
                                                        loadNext={this.loadNext.bind(this)}/>
                            )
                        }
