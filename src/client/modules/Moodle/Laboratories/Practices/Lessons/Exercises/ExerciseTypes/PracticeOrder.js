...
import OrderWordContainer from 'src/client/modules/Moodle/Laboratories/Practices/Lessons/Exercises/ExerciseTypes/PracticeOrder/OrderWordContainer'
import FrontTextsActions from 'src/client/modules/FrontTexts/FrontTextsActions'

export default class PracticeOrder extends React.Component {
  constructor() {
    super()
    this.state = {
      allExercise: [],
  ...
  loadPageTexts(){
    FrontTextsActions.getTexts("LAB_PRACTICE_EXERSICE_ORDER", (err, body) => {
      // si llega un error
      if (err) {
  ...
  checkExcercises() {
    ...
    for (let index in this.refs) {
      this.refs[index].checkIfFinished()
      practiceLength++
    }
  ...
  render() {
    ...
    let headerInfo = {
      ...
      // description: 'Coloca las palabras en el orden correcto para formar una oración.'
    }
    ...
    if (this.props.location.query.exerciseId < this.props.location.query.lengthOfExer) {
      next = Constants.ADMIN_PATH + `/user-area/practices/lessons/practice-order/?levelName=${this.props.location.query.levelName}&subLevelName=${this.props.location.query.subLevelName}&subLevelId=${this.props.location.query.subLevelId}&lessonName=${this.props.location.query.lessonName}&lessonId=${this.props.location.query.lessonId}&lessonType=${this.props.location.query.lessonType}&lessonIndex=${this.props.location.query.lessonIndex}&exerciseId=${nextExerId}&lengthOfExer=${this.props.location.query.lengthOfExer}`
    }
    ...
    return (
      ...
      <div className="col-xs-12">
              ...
              <div className="row">
                <div className="col-xs-12">
                  <span className="info-title-complete">{this.props.location.query.levelName}
                    ...
                    / Ordenar oraciones / Ejercicio {this.props.location.query.exerciseId}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {this.state.allExercise.map((exercise, index) => {
          return <OrderWordContainer key={index} practiceScore={this.practiceScore.bind(this)} exerciseIndex={index + 1} ref={"excercise" + index} phrase={exercise.question}/>
