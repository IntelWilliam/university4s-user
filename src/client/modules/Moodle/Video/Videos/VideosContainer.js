...
export default class VideosContainer extends React.Component {
    ...
    componentWillMount(){
      ...
      const SubLevel = this.props.subLevelIndex == 1
          ? "_i/"
      ...
      const videoIndex = videoHtml.substring( videoHtml.search("/lesson_") + 8 , videoHtml.search(".mp4"));

      const videoName = "lesson_" + videoIndex + ".mp4"
      const thumbName = "lesson_" + videoIndex + ".png"

      if (cat == "C"){
        var videoP = `/api/videos/` + pathLevel + videoName
        var thumbP = `/api/videos/` + pathLevel + thumbName
      }else{
        var videoP = `/api/videos/` + pathCat + pathLevel + videoName
        var thumbP = `/api/videos/` + pathCat + pathLevel + thumbName
      }
    ...
    playVideo() {
        this.props.playVideo.call(null, this.state.videoPath, this.props.video.spanish_statement, this.props.subLevelIndex, this.props.videoIndexOrLesson + 1 );
    }

    render() {
        return (
            <div className="col-xs-12 col-sm-3 card-container presentation-main">
                    <div className="card" id="videoCard" onClick={this.playVideo.bind(this)}>
                        <div className="image-container color-video-clase paddin-image">
                            ...
                            <img src={this.state.thumbPath} onError={this.imageNotFound.bind(this)}/>
                        </div>
