...
export default class Lesson extends React.Component {
  ...
  iframeChange(){
    ...
    setTimeout(() => { this.afterloadIframe() }, 100);
  }
  ...
  afterloadIframe(){
    ...
    { // FF
      iFrameBody = iFrame.contentDocument.getElementsByTagName('body')[0].children[0];
    }
    else if ( iFrame.contentWindow )
    { // IE
      iFrameBody = iFrame.contentWindow.document.getElementsByTagName('body')[0].children[0];
    }

    if(iFrameBody){
      document.getElementById("iframeBooks").style.height = iFrameBody.offsetHeight+"px";
    }
  ...
  render() {
    ...
    let sublevelIndex = this.props.location.query.subName[this.props.location.query.subName.length -1 ]

    console.log('sublevelIndex', sublevelIndex);
    let bookSrc = 'https://books.akronenglish1.com/' +
    this.props.location.query.levelName.toLowerCase() + '_' +
    this.romanize(parseInt(sublevelIndex))+ '/lesson_' +
    this.props.location.query.lessonIndex + '/1.html'
    ...
    let navigationArray = [
      {
        ...
        'name': subName,
        'url': Constants.ADMIN_PATH + `/user-area/course/lessons/?levelName=${this.props.location.query.levelName}&subLevelName=${subName}&subLevelId=${subId}`
      }, {
          ...
          <div className="row">
            <div className="col-xs-12">
              ...
              <div className="info-title-container">
                <span className="info-title tittle-blue">{this.props.location.query.lessonName}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="card-next-event col-xs-12">
            <div className="row">
              <div className="col-xs-12 header-card header-calendar">
                <span className="card-next-event-title">{this.state.pageTexts[3]}</span>
                {/* <span className="card-next-event-title">Navegación</span> */}
              </div>
              <div className="col-xs-12 body-card">
                <div className="col-xs-12">
                  <span className="card-next-event-title-body">{this.state.pageTexts[4]}</span>
                ...
                <div className="col-xs-12 navegation-container">
                  ...
                  <span className="navegation-2">{this.state.pageTexts[5]}</span>
                  {/* <span className="navegation-2">Cursos</span> */}
                </div>
                <div className="col-xs-12 navegation-container navegation-container-3">
                  <div className="arrow-down-navegation"></div>
                  <span className="navegation-2">{this.props.location.query.subName}</span>
                </div>
                <div className="col-xs-12 navegation-container navegation-container-4">
                  <span className="navegation-2">{this.props.location.query.lessonName}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="col-xs-12">
              <div className="row">
                <Link to={href} className="col-xs-12 action-container">
                  <button className="next-button mousePoint">{this.state.pageTexts[6]}</button>
