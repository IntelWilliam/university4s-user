...
class SectionItem extends React.Component {
      ...
      InteractionStore.getAll(params, (err, response) => {
        ...
        if (this.props.name == Constants.SECTION.VOCABULARY) {
          let pictureComments = response.data.filter((interaction) => {
            return (
              interaction.interactionType ==
                InteractionsConstants.IMAGE_COMMENT ||
              interaction.interactionType == InteractionsConstants.COMMENT
            );
          });
          ...
          let exercises = response.data.filter((interaction) => {
            return (
              interaction.interactionType > 3 && interaction.interactionType < 9
            );
          });
          ...
          this.props.name == Constants.SECTION.GRAMMAR ||
  ...
  render() {
    ...
    return (
      <div
        className='col-xs-12 col-md-3 front web-practice-sections'
        style={{ opacity: isSelect }}
      >
        {(() => {
          //freeAccess
          // if (true) {
          if (this.props.isUnlocked || this.props.isTeacher) {
            return (
              <span
                className={classSection + ' mousePoint'}
                onClick={this.loadInteractions.bind(this)}
              >
            ...
            return (
              <span className={classSection}>
                <div className='bloqued-big-container'>
                  <img
                    className='bloqued-big-enter'
                    src='/images/bloquedbig.png'
                  />
                </div>
                <span className='section-text'>{this.props.name}</span>
              </span>
