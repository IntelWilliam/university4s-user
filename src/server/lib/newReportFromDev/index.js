...
} from 'src/server/models/Certification'
...
export function accessLaboratory(params, callback) {
  ...
  if(params.category == "Marcar la Respuesta" || params.category == "reinforcement"){
    cat = 3
  }
...
export function approveDifficult(params, callback) {
  ...
  User.findOne({ 'userIdDev': userDevId }, (err, User) => {
    ...
    if (User) {
      request({
        uri: Constants.API_BASE_URL + 'user_data/' + userDevId,
        method: "GET"
      }, (err, response, body) => {
...
function checkNotifyAndSendMail (name, level, User){
  ...
  if(User.notifyCert && User.notifyCert == level){
    ...
    '<a href="http://admin.akronenglish1.com/certification/edit/' + User._id + '">Certificar Alumno</a>';
...
function updateCertification(newData, User) {
  ...
  Certification.findOneAndUpdate(query, { $set: newData }, { safe: true, upsert: true, new: true }, (err, certification) => {
    if (err) {
...
function checkNoteLevel(userNotes, User){
  ...
  if(userNotes.initial && userNotes.fundamental && userNotes.operational){
    ...
    if( parseInt(userNotes.initial.level_note) > 70
    ...
    }else if( parseInt(userNotes.fundamental.level_note) >= 70
    && parseInt(userNotes.operational.level_note) == 0){
