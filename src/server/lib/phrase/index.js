...
export function getPhrase(query, callback) {
  ...
  if ("lenguageId" in query && "different" in query) {
    ...
    Phrase.find(query).populate({ path: 'lenguageId', select: 'name _id' }).exec(function(err, phrases) {
...
export function addPhrase(newPhrase, callback) {
  ...
  if ("tosave" in newPhrase) {
    ...
    Promise.resolve(findOrRegisterPhrase(newPhrase))
      .then((res) => {
...
export function updatePhrase(id, newData, callback) {
  ...
  Phrase.findByIdAndUpdate(id, { $set: newData }, { safe: true, upsert: true, new: true }, (err, phrase) => {
    if (err) return callback(err)
...
export function addRelationPhrase(newPhraseTranslation, callback) {
  ...
  if ("newPhraseTranslation" in newPhraseTranslation) {
    // arreglo a guardar
    let newPhraseTranslationMod = JSON.parse(newPhraseTranslation.newPhraseTranslation)
...
export function getRelationPhrase(id, param, callback) {
  var idref = Types.ObjectId(id)
    // se buscan las frases y se hace un join (populate)
  PhraseTranslation.aggregate([{ $match: { phrases: idref } }, { $unwind: '$phrases' }, { $match: { phrases: { $ne: idref } } }]).exec(function(err, phraseTranslations) {
    PhraseTranslation.populate(phraseTranslations, { path: 'phrases', select: 'phrase _id lenguageId' }, function(err, populatedTransactions) {
