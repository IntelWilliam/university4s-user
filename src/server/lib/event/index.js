...
export function addEventsBooks(date, userId, callback) {
  getMoodleLessons((err, resp) => {
    ...
    let daysMonth = 30;
    let dateTemp = date ? new Date(parseInt(date)) : new Date();
    ...
    for (let month in arrLessonsMonth) {
      ...
      if (numberEntriesForDay == 0) {
        //Si numberEntriesForDay es cero es porque son muy pocas entries en un mes
      ...
      for (let lesson in arrLessonsMonth[month]) {
        if (isEntriesForDay) {
          //Se pregunta si se van a hacer entries por día, en caso contrario es días por entry
        ...
        let event = {
          //Se crea un nuevo evento con la fecha, la entrada, el usuario y todos los datos necesarios
          levelName: levelName,
...
export function getMoodleLessons(callback) {
  request(
    {
      uri:
        (process.env.API_BASE_URL || Constants.API_BASE_URL) + 'all_lessons/',
      method: 'GET',
    },
    (error, response, body) => {
...
export function addEvents(date, userId, callback) {
  Entry.find(
    ...
    .sort({ sublevelMoodleId: 1, lessonMoodleId: 1, sectionObjId: 1 })
    .exec((err, entries) => {
      ...
      for (let month in arrEntriesMonth) {
        ...
        for (let entry in arrEntriesMonth[month]) {
          ...
          if (arrEntriesMonth[month][entry].subWebSublevelId) {
            event['subWebSublevelId'] =
              arrEntriesMonth[month][entry].subWebSublevelId;
          }
          if (arrEntriesMonth[month][entry].lessonWebLessonId) {
            event['lessonWebLessonId'] =
              arrEntriesMonth[month][entry].lessonWebLessonId;
          }
          if (arrEntriesMonth[month][entry].levelWebLevelId) {
            event['levelWebLevelId'] =
              arrEntriesMonth[month][entry].levelWebLevelId;
          }
          events.push(event);
          numberEntries++;
        }
