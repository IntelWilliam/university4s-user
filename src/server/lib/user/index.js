...
import {getFirstSubLevel, getNextSubLevelFromArray} from 'src/server/lib/sublevel'
...
export function getUserDev(query, callback, full) {
  ...
  console.log('Constants.API_BASE_URL + \'login\'', Constants.API_BASE_URL + 'login');

  request({
    uri: Constants.API_BASE_URL + 'login',
    method: "POST",
    form: {
    ...
    User.findOne({
      ...
      if (!userFound) {
        ...
        let userTypeAux = userDev.userType == "student"
          ? "learner"
          : userDev.userType;
        var newUser = {
          userIdDev: userDev.id,
...
export function changeUserLesson(subLevel, user, callback) {
  ...
  getFirstLesson(subLevel._id, (lessonError, lesson) => {
    if (lessonError)
      return callback(lessonError)
    addUserLesson(user._id, subLevel._id, lesson._id, (userLessonError, userLessons) => {
      if (userLessonError)
        return callback(userLessonError);
...
export function changeUserPractice(lesson, user, callback) {
  ...
  getFirstPractice(lesson._id, (practiceError, practice) => {
    if (practiceError)
      return callback(practiceError)
    addUserPractice(lesson._id, user._id, practice._id, (userPracticeError, userPractices) => {
      if (userPracticeError)
        return callback(userPracticeError);
...
export function changeUserSection(practice, user, callback) {
  ...
  getFirstSection(practice._id, (sectionError, section) => {
    addUserSection(practice._id, user._id, section._id, (userSectionError, userSections) => {
      if (userSectionError)
        return callback(userSectionError);
      callback(null, user)
    })
  })
}
