...
router.get('/get-countries', (req, res) => {
  request({
    uri: Constants.API_BASE_URL + 'get_countries',
    method: "GET"
  }, (error, response, body) => {
...
router.get('/get-states', (req, res) => {
  ...
  request({
    uri: Constants.API_BASE_URL + 'get_states/' + '?a01Codigo=' + req.query.a01Codigo,
    method: "GET",
  }, (error, response, body) => {
    if (!error && response.statusCode == 200) {
      let jsonResponse = JSON.parse(body);
...
router.get('/get-provinces', (req, res) => {
  ...
  request({
    uri: Constants.API_BASE_URL + 'get_provinces/' + '?a02Codigo=' + req.query.a02Codigo,
    method: "GET",
  }, (error, response, body) => {
    if (!error && response.statusCode == 200) {
      let jsonResponse = JSON.parse(body);
...
router.get('/get-cities', (req, res) => {
  ...
  request({
    uri: Constants.API_BASE_URL + 'get_cities/' + '?a03Codigo=' + req.query.a03Codigo,
    method: "GET",
  }, (error, response, body) => {
    if (!error && response.statusCode == 200) {
      let jsonResponse = JSON.parse(body);
