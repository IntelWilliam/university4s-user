  ...
  validateCode(userData.codeId, (err, code) => {
    ...
    if (code) {
      ...
      if (code.stateCode === false) {
        ...
        // crea usuario en dev
        request({
          uri: Constants.API_BASE_URL + 'newAccount',
          method: "POST",
          form: {
            name: userData.name,
            lastname: userData.lastname,
          ...
          params['status'] = true
          params['role'] = "learner"
          params['username'] = params.email.slice(0, params.email.indexOf('@'));
          ...
          params['accountLevel'] =
            userCode[userCode.length - 1] == 'T' ? 1 :
              userCode[userCode.length - 1] == 'B' || userCode[userCode.length - 2] == 'B' ? 2 : 0;
