...
import { getInteraction, addInteraction, updateInteraction, removeInteraction } from 'src/server/lib/interaction'
...
router.get('/', (req, res) => {
  let query = req.query
  getInteraction(query, (err, response) => {
...
router.post('/', (req, res) => {
  let params = req.body
  addInteraction(params, (err, interaction) => {
...
router.get('/:id', (req, res) => {
  ...
  getInteraction(id, (err, response) => {
    if (err) return res.status(500).json({ error: err })
    res.json(response)
  })
})
...
router.put('/:id', (req, res) => {
  ...
  updateInteraction(id, params, (err, interaction) => {
    if (err) return res.status(500).json(err)
    res.json(interaction)
  })
})
